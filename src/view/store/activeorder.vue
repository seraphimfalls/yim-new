...
export default {
  ...
  async created() {
    ...
    if (
      ...
      const res2 = await storeApi.getMyLeftCount(this.adressId)
      console.log(res2)
    }

    //请求商品详情接口
    const res = await storeApi.getSpecialGoodsDetail(this.id);
    const res2 = await storeApi.getMyLeftCount(this.id)
    this.goodsNum = res2.data
  ...
  methods: {
    ...
    async order() {
       const res = await storeApi.createSpecialGoodsOrder(
          this.adressId,
          this.id,
          this.value
        );
